...
const BASE_URL = import.meta.env.VITE_BASE_URL
    ...
    export default {
        ...
        mounted: async function () {
            ...
            const response = await axios.get(`${BASE_URL}/grades/studentgrades/${this.$route.params.student_id}`);
        ...
        methods: {
            async addCourse (event) {
                ...
                let response = await axios.put(`${BASE_URL}/students/${this.$route.params.student_id}/`, {courseId: this.newCourse})
                let gradeRes = await axios.post(`${BASE_URL}/grades/${this.$route.params.student_id}/${this.newCourse}`)
            },
            handleChange (event) {
        ...
        <div class="card mt-5 mx-auto" style="width: 18rem;">
            ...
            <ul class="list-group list-group-flush">
                <div v-for="item in student">
                    ...
                    <li v-if="iterator" class="list-group-item">{{ iterator.studentId.name }}</li>
                    <li v-if="iterator" class="list-group-item">{{ iterator.studentId.email }}</li>
                    <li v-if="iterator" class="list-group-item">{{ gpa }}</li>
            </ul>
        </div>
        <form @submit="addCourse">
            <select @change="handleChange" class="form-select mt-5 mx-auto" aria-label="Default select example" style="width: 18rem;">
        ...
        <div class="card mt-5 mx-auto" style="width: 24rem;">
            ...
            <ul class="list-group list-group-flush">
                <div v-for="course in student">
                    <li class="list-group-item d-flex justify-content-between"><span>{{course.courseId.name}}</span> <span>{{ course.letter}}</span></li>
                </div>
